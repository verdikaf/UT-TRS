...
// Falls back to a placeholder string if unset (will throw on import).

const PUBLIC_KEY_PEM = (import.meta.env.VITE_RSA_PUBLIC_KEY || '').trim();
let importedKey = null;

function pemToArrayBuffer(pem) {
  const b64 = pem.replace(/-----BEGIN PUBLIC KEY-----|-----END PUBLIC KEY-----|\r|\n|\s/g, '');
...
export async function encryptPassword(plain) {
  ...
  const cipherBuf = await crypto.subtle.encrypt({ name: 'RSA-OAEP' }, importedKey, encoded);
